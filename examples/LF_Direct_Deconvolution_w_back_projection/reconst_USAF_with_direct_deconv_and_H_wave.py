...
# Set the colormap for image outputs
colormap = 'gray'
...
# Save the current script and settings file in the results directory for reproducibility
...
# Define the input directory containing irradiance images
...
for file_path in input_file_paths:
    image = np.load(file_path)  # Load each image
...
# Compute scaling factor for resizing the calibration image
...
# Compute PSF size for the micro-lens array
...
# Define image dimensions and coordinate grids
...
# Magnification factor and spatial shifts for the lightfield
...
# Define grids for shifted coordinates
...
# Arrays for storing projections and reconstructed images
forward_projections = []
reconstructed_images = []

# Iterate over each depth in the lightfield
for idx in tqdm.tqdm(range(object_coordinates.shape[2])):
    # Extract and process object coordinates for the current depth
    ...
    # Resize and pad wave propagation kernel to match calibration scaling
    ...
    # Define backward propagation function for reconstruction
    ...
    # Load and preprocess irradiance image
    ...
    # Perform reconstruction using the backward function
    reconstructed_wave = backward_function(zero_padded_image)

    # Save the reconstructed and raw rectified images
